...
export function SolvedWordRow({ ...props }) {
  ...
  return (
    <animated.div style={springProps}>
      {!isImageAvailable ? (
        <div style={{ backgroundColor: color, borderRadius: 8, textAlign: "center", fontWeight: "700", fontSize: "20px", height: "80px"}}>
          <p className="font-bold pt-2 pl-4" style={{}}>{props.category.toUpperCase()}</p>
          <p className="font-thin pb-2 pl-4" style={{fontWeight: "400"}}>{props.words.join(", ").toUpperCase()}</p>
        </div>
      ) : (
        <Popover>
          <PopoverTrigger asChild>
            ...
            >
              ...
              <p className="font-bold pt-2 pl-4">{props.category.toUpperCase()}</p>
              <p className="font-thin pb-2 pl-4">{props.words.join(", ").toUpperCase()}</p>
            </div>
          </PopoverTrigger>
          <PopoverContent>
            <div>
...
function GameGrid({ gameRows, shouldGridShake, setShouldGridShake }) {
  const { submittedGuesses, isGameOver, isGameWon, solvedGameData } =
    React.useContext(GameStatusContext) ?? {submittedGuesses: [], isGameOver: false, isGameWon: false, solvedGameData: []};
  console.log("GameGrid -> solvedGameData", solvedGameData)
